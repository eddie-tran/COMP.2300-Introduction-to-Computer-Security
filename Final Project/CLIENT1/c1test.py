...
def get_user_data():
    ...
    # getpass makes it so password input is invisible on screen
...
#        passwd = getpass.getpass("Enter Password: ")
    # have user input password again
    ...
    while True:  # loop until the user correctly inputs the intended password two times to confirm the user's password
        if passwd_check == passwd:  # match
            ...
            hashed_passwd = bcrypt.hashpw(
                passwd, bcrypt.gensalt(10))  # apply bcrypt hash
            hashed_passwd = hashed_passwd.decode(
                'utf-8')  # decode to apply to json
            ...
            # let user reenter password again after failing the password check
            passwd = getpass.getpass("Enter Password: ")
            passwd_check = getpass.getpass("Re-enter Password: ")
    return user_data
...
def login():
    ...
    while (is_user == False):
        email = input("Enter Email Address: ")
        passwd = getpass.getpass("Enter Password: ")
        # authenticate the user by checking if email and password input matched with the user in the json file
        is_user = authenticate_user(email, passwd)
...
def authenticate_user(email, passwd):
    with open("user.json", "r") as f:  # open json file as read
        ...
        for user in file_data["user_info"]:
            if email == user["email"]:
                # passwords have to be encoded to check if passwords matched
...
def add(s, host, port):
    ...
    # checks if there is anything in the contacts json file
    if os.path.getsize(file_path) == 0:
        # if there is nothing, will create a new dictionary and append first key-value pair
        ...
        with open("contacts.json", "r+") as f:
            ...
            # already have a contact?, check if contact exists
            for e in file_data["contact_info"]:
                if email == e["email"]:  # email address is used as the user identifier
                    ...
                    # contact does exist, so overwrite the data with the new contact data
                    e.update(new_user)
                    f.seek(0)
                    json.dump(file_data, f, indent=4)
                    f.truncate()
    ...
    if os.path.getsize(file_path) != 0:
        ...
        with open("contacts.json", "r") as f:
            ...
            for e in file_data["contact_info"]:
                ...
                msg = msg.encode("utf-8")
                # the method transmits UDP message ## sendto(bytes, address)
                s.sendto(msg, (host, port))
                ...
                # the method transmits UDP message ## sendto(bytes, address)
                s.sendto(contact_email, (host, port))
...
def send(email, file):
    ...
    # send file
    t = socket.socket(socket.AF_INET,socket.SOCK_STREAM) ## connection for sending the file
    t.bind((host,port))
    t.listen()
    ...
    while True:
        if file_data:
            ...
            break
    with open(r"C:\Users\joshh\Desktop\airdropv2" + '\server_list.json') as f:
        file_data2 = json.load(f)
        for e in file_data2["server_info"]:
            # check email to see if users is online
            if (email == e["email"]):
                print("Matches.") # Testing
                # if user is online send prompt and await response
                    # send flag to server to request to send to target client
                msg = "Send Permission"
                msg = msg.encode("utf-8")
                s.sendto(msg, (host, port))
                # send email
                msgemail = email
                msgemail = msgemail.encode("utf-8")
                s.sendto(msgemail, (host, port))
...
def encryption():
    ...
    [file_out.write(x) for x in (encrypted_key_aes,
...
def decryption():
    ...
    encrypted_key_aes, nonce, tag, ciphertext = [file_in.read(
...
def main():
    ...
    if check_existing_user() == False:
        ...
        # after successful login, proceed to the interface
        interface(s, host, port)
