...
def interface():
    ...
    while True:
        ...
        if command == "help":
            ...
            remove_user() # Remove user from server list
            break
...
def list(): 
    ...
    with open("../NETWORK/server_list.json", "r+") as f:
        server_data = json.load(f)
        for e in server_data["server_info"]:
            print("  * "+ "<"+e["email"]+">")
...
# secure_dropy program is started, open the udp client
        ...
        #host = hostbyname # localhost
        host = hostbyname
        port = 55555 # port number
        with open("user.json", "r") as f:
            file_data = json.load(f)
            user = file_data["user_info"][0]["email"]
        #msg = user + " is online!"
        msg = user
...
def remove_user():
    with open("user.json", "r") as f:
        ...
        user_email = user_data["user_info"][0]["email"]

    #updated_user = {"email": "", "port": ""}
    with open("../NETWORK/server_list.json", "r+") as u:
        server_data = json.load(u)
        count = 0
        for e in server_data["server_info"]:
            if e["email"] == user_email:
                print("  Leaving the Server... ")
                server_data["server_info"].pop(count)
                #e.pop("email")
                #e.pop("port")
                #del e["email"]
                #del e["port"]
                #e.update(updated_user)
                u.seek(0)
                json.dump(server_data, u, indent = 4)
                u.truncate()
            elif e["email"] != user_email:
                count += 1
            else:
                print(" Server User Doesnt Exist. ")
...
def main():
    ...
    if check_existing_user() == False:
        ...
        add_user_to_server()
        interface()  # after successful login, proceed to the interface
